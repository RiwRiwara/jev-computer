...
```
Jev API 1 ครั้ง ──► ตาราง NAND ──► full adder (NAND 9 ตัว) ──► ALU ──► CPU 8-bit + RAM
  446 tokens       p = .00 .01 .01 .99      NAND 2,102 ตัว ลึก 59 ชั้น
```
...
| `--sim` | ใช้ gate อุดมคติ ไม่เรียก API (ฟรี ใช้ลองก่อน) |
| `--live` | ถาม Jev ใหม่ทุกชั้นของวงจร ทุกรอบ clock (ช้ามาก ดูด้านล่าง) |
...
- **โหมดปกติ (fast) จ่ายเท่ากันทุกโปรแกรม** คือ 1 request, 446 tokens เพราะ Jev ตอบตาราง gate ครั้งเดียว แล้วทุก gate ใช้คำตอบนั้นร่วมกัน
- **โหมด `--live` ยิ่งรอบ clock มาก ยิ่งแพงและนาน**
  - รอบ clock ละ 59 requests ใช้เวลาราว 49 วินาที
  - `add a=40 b=3` ใช้ 5 รอบ, 295 requests, 245 วินาที
  - `mul` หรือ `div` จะใช้เวลาหลายชั่วโมง ถ้าจะโชว์โหมด live ให้ใช้ `add` หรือ `sub`
- **ลองด้วย `--sim` ก่อน แล้วค่อยรันกับ Jev จริง** และใช้ `--trace` ดูว่าคำตอบคำนวณออกมาอย่างไร
...
| ชุดทดสอบทุกโปรแกรม (โหมด fast) | ผ่าน 19/19 | 19 | 8,474 | ~15 วินาที |
| `add a=40 b=3` แบบ **live** (ถาม Jev ทุกชั้น ทุกรอบ clock) | 43 | 295 | 112,441 | 245 วินาที |
| Countdown แบบ **live** (prompt รุ่นเก่า 1 request ต่อ 1 กรณี) | 5 4 3 2 1 0 | 3,642 | ไม่ได้วัด | 1,175 วินาที |
...
3. **ตัวรัน:** `jev_run.py` โหลดโปรแกรม JSON ลง RAM แล้วประมวลผลวงจรทีละชั้นด้วยคำตอบของ Jev และเก็บ state ใหม่ทุกรอบ clock
4. **สองโหมด**
   - *fast* (ค่าเริ่มต้น): Jev ตอบ 4 กรณีครั้งเดียวต่อการรัน แล้วทุก gate ใช้คำตอบนั้น
   - *live*: ถาม Jev ใหม่ทุกชั้นของวงจร ทุกรอบ clock เพื่อพิสูจน์ว่า Jev อยู่ใน loop จริง
