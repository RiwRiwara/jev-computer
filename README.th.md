...
| `--cpu=ชื่อรุ่น` | รันบน CPU รุ่นอื่น: `mini`, `classic` หรือ `plus` (ดู [รุ่นของ CPU](#รุ่นของ-cpu)) |
...
มีเครื่อง 3 รุ่น ทุกรุ่นใช้คำถาม gate เดียวกันและรูปแบบคำสั่งเดียวกัน โปรแกรมใน `programs/` จึงรันได้ทุกรุ่นที่ RAM พอ

| รุ่น | ไฟล์ | RAM | NAND gate | จำนวนคำสั่ง | `prime n=251` |
|---|---|---|---:|---|---:|
| `mini` | `cpus/mini/cpu.json` | 64 bytes | 6,477 | 11 | 1.9 วินาที |
| `classic` (ค่าเริ่มต้น) | `cpu.json` | 256 bytes | 24,511 | 11 | 4.8 วินาที |
| `plus` | `cpus/plus/cpu.json` | 256 bytes | 32,737 | 16 | 7.0 วินาที |

ทุกรุ่นจ่ายเท่ากัน คือ 1 request, 446 tokens เพราะ Jev ตอบแค่ 4 กรณีของ gate เสมอ เครื่องที่ใหญ่ขึ้นแค่มี gate ให้ประมวลผลในเครื่องเรามากขึ้น

```bash
python jev_run.py programs/prime.json n=251 --cpu=mini          # เลือกรุ่นเอง
python jev_run.py cpus/plus/programs/mul_fast.json a=1 b=200    # โปรแกรมใน cpus/plus/ ใช้รุ่น plus ให้อัตโนมัติ
```

- **`mini`** ใส่โปรแกรมได้ 13 จาก 15 ตัว ส่วน `bubble_sort` (104 bytes) กับ `max_min` (69 bytes) ใหญ่เกิน ตัวรันจะแจ้งให้
- **`plus`** เพิ่มคำสั่ง 5 ตัว:

| คำสั่ง | ความหมาย |
|---|---|
| `LDX n` | A = RAM[RAM[n]] (อ่านผ่าน pointer) |
| `STX n` | RAM[RAM[n]] = A (เขียนผ่าน pointer) |
| `AND n` | A = A AND RAM[n] ทีละ bit |
| `SHL` | เลื่อน A ไปทางซ้าย 1 bit ได้ carry = bit ที่หลุดออกไป |
| `SHR` | เลื่อน A ไปทางขวา 1 bit ได้ carry = bit ที่หลุดออกไป |

โปรแกรมสำหรับ `plus` อยู่ใน `cpus/plus/programs/`:

| ไฟล์ | ทำอะไร | ตัวอย่าง |
|---|---|---|
| `mul_fast.json` | a × b แบบเลื่อน bit แล้วบวก | `a=1 b=200` → 200 ใน 114 รอบ clock (`mul` ใช้ 1,805 รอบ) |
| `popcount.json` | นับจำนวน bit ที่เป็น 1 ใน n | `n=170` → 4 |
| `bubble_sort.json` | เรียงตัวเลข 6 ตัวด้วย pointer แทนการแก้คำสั่งตัวเอง | `list=5,2,9,1,7,3` → [1, 2, 3, 5, 7, 9] |
| `max_min.json` | ค่ามากสุดและน้อยสุดของตัวเลข 8 ตัว ด้วย pointer | → [200, 3] |

ชุดทดสอบของ `plus` ผ่านกับ Jev จริงครบ 19 เคส

ถ้าจะเพิ่มรุ่นใหม่ ให้เพิ่มรายการใน `MODELS` ของ `build_cpu.py` แล้วรัน `python build_cpu.py ชื่อรุ่น`
...
| `cpu.json` | เครื่องรุ่น classic: NAND 24,511 ตัว + ชุดคำสั่ง (สร้างอัตโนมัติ) | `jev_run.py` อ่านไฟล์นี้เอง |
| `cpus/` | CPU รุ่นอื่น (`mini`, `plus`) และโปรแกรมของ `plus` | ใช้ `--cpu=ชื่อรุ่น` หรือรันโปรแกรมจาก `cpus/plus/programs/` |
| `build_cpu.py` | สร้าง `cpu.json` ของแต่ละรุ่น ส่วน `--check` ใช้ตรวจวงจรโดยไม่เรียก API | เฉพาะตอนแก้หรือเพิ่มรุ่นเครื่อง |
...
ตอบตรงๆ ก็ไม่รู้เหมือนกัน 555 เริ่มจากสงสัยว่า "Jev บวกเลขได้ไหม" คำตอบคือ "ก็ได้แหละ แต่ไม่ค่อยชัวร์" รู้ตัวอีกที NAND ไปสองหมื่นกว่าตัว ได้คอมพิวเตอร์ที่ต้องยิง API ไปถามก่อนว่า 1 AND 1 ได้เท่าไหร่

แต่หนังวันสิ้นโลกทุกเรื่องก็เริ่มจากมีคนสอน AI นับเลขนี่แหละ วันนี้มันคิด 5! ได้ พรุ่งนี้… ก็ยังคิดได้แค่ 5! อยู่ดี เพราะ 6! ล้น 8 bit มนุษยชาติยังปลอดภัย (ไปก่อน) 🤖
